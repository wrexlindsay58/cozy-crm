  ...
  return (
    <section className="rounded-md border border-line bg-card p-4">
      ...
      <p className="mt-2 text-[11px] text-muted">
        {proposal.proposalStatus === "Draft" ? "No proposal yet. Generate from the options on this file." : `Status: ${proposal.proposalStatus}${proposal.signStatus !== "—" ? ` · Sign ${proposal.signStatus}` : ""}`}
      </p>
      {docs.length === 0 ? null : (
            ...
            <li key={d.id} className="rounded-md border border-line px-3 py-2 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold">
                  {d.kind === "proposal" ? "Proposal" : "Agreement"} {d.id}
                </span>
                <span className="text-[11px] font-bold tracking-wide text-muted uppercase">{d.status}</span>
              </div>
              {d.totals?.length ? (
                <p className="mt-1 text-[11px] text-muted">{d.totals.map((t) => `${t.name} ${money(t.amount)}`).join(" · ")}</p>
              ) : null}
            </li>
          ))}
        </ul>
