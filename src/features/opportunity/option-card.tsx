...
import { acceptOption, addLine, optionTotal, removeLine, removeOption, renameOption, setQty, toggleLine, type OptCard, type Proposal } from "./store";
        ...
        {option.lines.map((l) => (
          <li key={l.sku} className="flex items-center gap-2 text-sm">
            <button
              type="button"
              disabled={locked}
              onClick={() => toggleLine(proposal.oppId, option.id, l.sku)}
              className={cn(
                "grid size-9 shrink-0 place-items-center rounded border text-xs font-bold",
                l.on ? (accepted ? "border-card bg-card text-navy" : "border-navy bg-navy text-card") : "border-line",
              )}
              aria-pressed={l.on}
            >
              {l.on ? "On" : "Off"}
