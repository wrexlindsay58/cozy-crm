...
import { addPhoto, usePhotos } from "@/features/photos/store";
    ...
    <section className="rounded-md border border-line bg-card p-4">
      ...
      <form
        ...
        onSubmit={(e) => {
          ...
          if (file) {
            ...
            reader.onload = () => {
              addPhoto(personId, caption, typeof reader.result === "string" ? reader.result : undefined);
              setCaption("");
              setFileName("");
      ...
      >
        ...
        <input
          ...
          placeholder={fileName ? "Caption" : "Caption, or pick a file"}
      ...
      {photos.length === 0 ? <p className="mt-3 text-sm text-muted">No photos yet.</p> : null}
      <ul className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3">
        {photos.map((p) => (
          <li key={p.id} className="overflow-hidden rounded-md bg-page">
            {p.src ? (
              <img src={p.src} alt={p.caption} className="h-28 w-full object-cover" />
            ) : (
